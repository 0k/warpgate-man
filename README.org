...
If even that is too much to remember, =odoo-get-ssh-config= does the whole
thing on a machine that has nothing installed. It checks for =uv= (and the
=git=/=curl= it needs), offers to install what is missing, asks once for
your Odoo server, database, login and bastion address, saves them to
=~/.config/wgman/config.yaml=, and prints the ssh config.

It is a single self-contained script, so you need no clone to get it:
...
Read it before running it, as with anything you download. To keep it
around, move it onto your =PATH= (=~/.local/bin= is the usual spot):

#+begin_src sh
mv odoo-get-ssh-config ~/.local/bin/
#+end_src

For a true one-liner, pipe it straight to =bash= — its prompts are read
from the terminal, not stdin, so they still work when the script itself
arrives on stdin:

#+begin_src sh
curl -LsSf https://raw.githubusercontent.com/0k/warpgate-man/master/scripts/odoo-get-ssh-config \
    | bash > ~/.ssh/config.d/warpgate
#+end_src

From a checkout it is =./scripts/odoo-get-ssh-config=, with no
difference in behaviour.

Only the ssh config goes to stdout — every question and progress line
goes to stderr, and the prompts are read from the terminal — so the
redirection above works while the script is still interactive. Later runs
reuse the saved answers and ask only for your Odoo password, which is
never written to disk (=--reconfigure= to change them).

Nothing is installed without asking; =--yes= accepts in advance and
=--no-install= refuses outright, failing with the name of what is
missing. Run =--help= for the rest.
